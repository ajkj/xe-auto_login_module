...
class auto_loginController extends auto_login {
	 ...
	 * @return BaseObject : AutoLogin을 진행하고 문제가 없을경우 new Object를 return 합니다.
	 */
	 ...
	 * @return BaseObject : 비정상적인 경우를 제외하고 $this->createObject() return;
	 */
	 ...
	 * @return BaseObject : from member.controller.php after doLogin Trigger
	 */
	 ...
	 * @return BaseObject : 에러를 제외하고 new Object return;
	 *
	 */
	private function makeAutoLogin($logged_info)
	{
		// 자동로그인이 아니면 나가기.
		$keep_signed = Context::get('keep_signed');
		if(empty($keep_signed) === true) return $this->createObject();

		// logged_info가 비어있으면, 가져오기
		if(!isset($logged_info))
		{
			$logged_info = Context::get('logged_info');
			if(isset($logged_info) === false) return $this->createObject();
		}
		...
		while(true)
		{
			...
			$i++;
			if ($i > 1){
				return $this->createObject(-1, "AutoLogin Erorr Occured");
			}
	...
	private function removeAutoLoginToken()
	{
		...
		if(isset($cookie)){
			...
			{
				return $this->createObject(-1,'AutoLogin Remove Error');
			}
		}
	 ...
	 * @return BaseObject : Erorr 가 있는 경우를 제외하고 빈 Object 생성.
	 */
	private function doAutoLogin(){
		...
		{
			return $this->createObject(-1, 'AutoLogin Error : Controller');
		}
	...
	{
		if(checkCSRF() !== true) return $this->createObject(-1, "CSRF ERROR");
		...
		{
			return $this->createObject(-1, "Error : procAuto_loginRemoveAutoLoginToken Empty Session");
		}
		...
		{
			return $this->createObject(-1, 'AutoLoginToken Remove Error');
		}
